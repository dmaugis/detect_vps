...
if isempty(vertical_vps);
   ...
   % error('Sorry, I could not find the zenith')
end
...
vertical_vp = vertical_vps(:,vpI);%  sum(vertical_vps.*NFAs_vertical_vps_weights,2);
...
vertical_vp_unit = [vertical_vp(1)-pp(1);(H- vertical_vp(2))-(H-pp(2)); FOCAL_RATIO*W];
% normalize vectors;
vertical_vp_unit = vertical_vp_unit/norm(vertical_vp_unit);

horizontal_vps_unit = [horizontal_vps(1,:)-pp(1);(H- horizontal_vps(2,:))-(H-pp(2)); ones(size(horizontal_vps(2,:)))*FOCAL_RATIO*W];

%% check orthogonality of horizontal vps with vertical vp
orthogonality_scores = [];
for i=1:size(horizontal_vps_unit,2)
    % normalize
    horizontal_vps_unit(:,i)=horizontal_vps_unit(:,i)/norm(horizontal_vps_unit(:,i));
    
    orthogonality_score = abs(dot(horizontal_vps_unit(:,i), vertical_vp_unit));
...
if params.PRINT
    ...
    img = draw_segments(img,[horizontal_vps vertical_vp], lines_lsd, params);
